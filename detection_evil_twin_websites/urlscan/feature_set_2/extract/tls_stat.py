...
class Tls_stat:
    ...
    def __init__(self, urlscan_dict: Dict):
        ...
        try:
            self.tlsStats = urlscan_dict['stats']['tlsStats']
        except KeyError:
        ...
        self.absolute_dict['len'] = len(self.tlsStats)

    def compute_features(self):
        if self.data_available:
            for stat in self.tlsStats:

                for category_feature in self.categorical_features_names:
                    try:
                        res_dict = stat[category_feature]
                    except KeyError:
                        res_dict = {}
                    for tls_name, value in res_dict.items():
                        if self.categorical_dict[category_feature].get(tls_name, None) is None:
                            self.categorical_dict[category_feature][tls_name] = value
                        else:
                            self.categorical_dict[category_feature][tls_name] += value
