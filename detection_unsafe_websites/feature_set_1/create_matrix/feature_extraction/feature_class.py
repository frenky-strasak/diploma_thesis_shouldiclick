...
from feature_extraction.compute_json import ComputeJson
...
class UrlJsonClass(ComputeJson):
    ...
    def get_39_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['resourceStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_53_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['protocolStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_63_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['tlsStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_75_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['serverStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_85_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['domainStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_101_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['regDomainStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
    ...
    def get_113_size_mean(self) -> float:
        try:
            count_list = [stat['https'] if stat['https'] is not None else 0 for stat in self.json_dict_stats['ipStats']]
            return self.check_nan(np.mean(count_list))
        except KeyError:
            return float(-1)
